...
コーポレート共通のタグラインは持たない（**2026-09-06 オーナー指示**）。
「人から人へ渡る喜びを」は**ブランドリッツ（BtoC）のキャッチコピー**であり、
**社名（株式会社Ritz Solution Partner）の後ろにこの一文を並べて書かない**。
他ブランド（ネクストプラチナム・色石マーケット）の投稿に転用もしない。
会社としての一文が要る場合は、原典（会社案内パンフレット）のミッションかビジョンを**一字一句変えずに**使う。

- ミッション: 「私たちは、リユース業を通じてすべての人に、新たな価値観、新たな発見、新たな喜びを提供します。」
- ビジョン: 「私たちは、使われなくなったモノに新たな価値を与え循環させることで、持続可能な社会の実現を目指します。」

値の正本は `instagram/accounts.json`（キャッチコピーは `accounts.brand-ritz.catchcopy`、
会社の一文は `common.mission` / `common.vision`）。
