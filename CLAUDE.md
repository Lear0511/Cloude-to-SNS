...
**ネクストプラチナムの Instagram アカウントは brand-kit に記載が無く、実アカウントも未確認。**
`instagram/accounts.json` の `handle` / `url` は `null` のままにしてあり、検証で WARN が出る。
実アカウントを確認するまで**推測で埋めない**（WARN が出続けるのは意図した挙動）。

**色石マーケットは確認済み**（`@iroishi.market`。オーナー提供のスクリーンショット・2026-09-06）。
値の正本は `instagram/accounts.json` で、その `_handle_note` に出典と確認日を書いてある。
brand-kit にはまだ記載が無いので、**brand-kit 側への反映は別途行う**。
...
Facebook ページと連携されている必要がある。色石マーケットは実アカウントを特定できた段階で、
**プロアカウントか否かとページ連携は未確認**。ネクストプラチナムは実アカウント自体が未確認。
**運用開始前にプロアカウント化・ページ連携の確認が要る**（ネクストプラチナムはアカウントの特定から。手順は `SETUP.md`）。
