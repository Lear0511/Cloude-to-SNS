...
Instagram Graph API を使わず、**Web版 Claude で投稿案をつくり、Meta Business Suite で予約投稿する**運用の実装。

予約投稿ツールは **Meta Business Suite に一本化する**（無料で、追加費用なしに3ブランド分を回せるため。他ツールとの比較は「6. Meta Business Suite で予約投稿する」の末尾）。
...
| 3 | 検証して変換（`out/` に チェックシート・キャプション・一覧CSV） | コマンド |
| 4 | `out/*-review.md` のチェックボックスを人が通す | 手作業 |
| 5 | 「画像の指示」どおりに画像を作り、`media` 欄に記入 | 手作業 |
| 6 | Meta Business Suite のプランナーで予約投稿する | 手作業 |
...
| `posts[].hashtag_placement` | `caption`（**既定**）または `first_comment`。Meta Business Suite は1件目のコメントを自動投稿できないため、既定はキャプション本文に入れる。`first_comment` を指定すると検証で WARN が出て、公開後に人が手でコメントを入れる運用になる |
| `posts[].hashtags` | `#` 始まりの配列 |
| `posts[].media` | 画像ができたらファイル名やURLを入れる（一覧CSVに出る） |
...
| `hashtag_placement` が `first_comment`（Meta では公開後の手作業になる） | WARN |
...
| `out/<batch>-captions/NN-slug.txt` | **キャプション全文。Meta Business Suite にはこれを貼る**（実際の投入経路） |
| `out/<batch>-schedule.csv` | **バッチ全体の一覧・記録用。** 何をいつ出すかを1投稿1行で確認する。予約投稿ツールへの取り込みには使わない（将来ツールを乗り換えるときの素材として残す） |
...
カルーセルはアップロードした順に並ぶので、**ファイル名を `01-` `02-` と枚数順の連番にしておく**と手順6で並べ替えずに済む。

できた画像は `posts/*.json` の `media` に記入して `build-posts.py` を再実行すると、`review.md` と一覧CSVに反映される。

## 6. Meta Business Suite で予約投稿する

投入するものは `out/<batch>-captions/NN-slug.txt`（キャプション本文と、あれば1件目のコメント）と、手順5で作った画像。
`review.md` を横に開いて、alt と画像の並び順を見ながら進める。

### 手順

1. Meta Business Suite を開き、**投稿するブランドのビジネスポートフォリオに切り替える。** 別ブランドのアカウントが選択された状態で作業を始めないこと
2. プランナー → 「投稿を作成」
3. 投稿先で、**そのブランドの Instagram アカウントだけにチェックを入れる。** 他アカウントのチェックが残ったまま予約すると、そのまま誤投稿になる
4. 画像をアップロードする。カルーセルはアップロード順に並ぶので、`review.md` の「画像の指示」表の**上から順に**追加する
5. `out/<batch>-captions/NN-slug.txt` を開き、**`----- 1件目のコメント -----` より上をそのままキャプション欄に貼る。** ハッシュタグは既定でこの範囲の末尾に含まれているので、別途足さない。区切り線より下がある投稿は、公開後に手でコメントとして入れる（次節）
6. 画像1枚ごとに代替テキストを入れる。文言は `review.md` の alt をそのまま使う
7. 予約日時を `publish_at`（JST）に合わせる。**アカウントのタイムゾーン設定が日本時間になっているかを初回だけ確認する**（ずれていると全投稿が同じ幅でずれる）
8. 公開前にプレビューを開き、日時・画像の並び・本文の対応を目視する
9. 予約する。バッチの残りの投稿について 2〜9 を繰り返す

### ハッシュタグ

**Meta Business Suite には1件目のコメントを自動投稿する機能が無い。**

- 既定では `hashtag_placement` を `caption` にし、ハッシュタグをキャプション本文に入れる。手順5で貼る範囲に最初から入っているので、追加の作業は要らない
- `first_comment` を指定した投稿は検証で WARN が出る。予約の時点では入れられないため、**公開後に人が手でコメントを入れる**
- captions ファイルの `----- 1件目のコメント -----` より下（`first_comment` に書いた補足や法定表記、`first_comment` 指定のハッシュタグ）が、そのコメント本文。**区切り線より下があるかどうかを予約時に控えておき、公開後に貼る**

### 管理画面側の制限（未確認）

**予約できる先の期間の上限・1度に予約できる件数・カルーセルの最大枚数といった Meta Business Suite 側の制限は、本ドキュメント作成時点（2026-08-28）に公式ヘルプを参照できず裏が取れていない。**
推測の数値は書かない。**初回運用時に実機で確認し、分かった値をここに追記すること。**

### 将来ツールを乗り換える場合

2026-08-28 時点で各社公式ページを確認した比較。**この規模（Instagram 3アカウント・週8本前後）では Meta Business Suite で足りる**と判断した。

| ツール | 料金 | 今回の用途での評価 |
|---|---|---|
| **Meta Business Suite** | 無料 | **採用。** 追加費用なしで3ブランド分を回せる。1件目のコメントの自動投稿は不可 |
| Buffer | 1チャンネル（＝SNSアカウント1つ）ごとの課金。Essentials $5/チャンネル/月 → 3アカウントで $15/月 | 有料プランは1件目のコメント予約に対応。コメント自動化が必要になったら候補 |
| Later | 1つの「ソーシャルセット」に Instagram は1つしか入らず、3アカウントには3セット必要。2セットの Growth では足りず Scale $110/月（月払い） | 今回の用途には割高 |
| Hootsuite | Standard $99/月（年払い）から | 今回の規模には過剰 |

乗り換えるときの素材は `out/<batch>-schedule.csv`。列はこの順で出る。

```
publish_at_jst, account, handle, type, theme, caption, first_comment, media, alt_texts, slide_count
```

一括取り込みのCSV仕様はツールとプランで変わり、ここでは列名を検証していない。
移行するときは、**使うツールの取り込み画面からテンプレートCSVをダウンロードして列名と日時書式を突き合わせ**、必要なら列を並べ替えてから読み込むこと。
...
└── out/                 生成物（review.md / captions/ / schedule.csv）
```
